    ...
    def find_reason(self):
        ...
        if self.error_description:
            if self.error_description.find('Name or service not known') > -1:
                extra_details = None
                if self.options:
                    extra_details = 'The url requested was %s' % (term_colours.make_blue(self.options.api_url + self.options.path))
                ...
                extra_details = None
                if self.options:
                    extra_details = 'The url requested was %s' % term_colours.make_blue(self.options.api_url)
...
class BlueSkyApiTest(object):
    ...
    def __init__(self, options):
        ...
        if self.options.data_transport.lower() == 'xml':
            self.server = xmlrpclib.Server(self.options.api_url + self.options.path + self.xdebug_param, verbose=self.options.verbose_request)
        elif self.options.data_transport.lower() == 'json':
            self.server = jsonrpclib.Server(self.options.api_url + self.options.path + self.xdebug_param, verbose=self.options.verbose_request)
    ...
    def get_methods_available(self):
        ...
        try:
            if self.options.data_transport.lower() == 'xml':
                ...
                self.json_methods_available = json.loads(urllib.urlopen(self.options.api_url + self.options.path + self.xdebug_param).read())
...
def main():
    ...
    parser.add_option('-r', '--url',      action='store',      dest='api_url',         help='API url to query.',                            default='https://dev-deifante-blue.istockphoto.com')
